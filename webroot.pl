...
sub displayJournal {
	...
	if ($key = $root_key->get_subkey($key_path)){
		::rptMsg("");
		::rptMsg($key_path . " - ". ::getDateFromEpoch($key->get_timestamp())." (UTC)");
		::rptMsg("");
		...
		foreach my $val (@vals) {
			...
			my $md5= (split(/\=/,$d[1]))[1];
			my $ts=(split(/\=/,$d[2]))[1];
			my $timestamp=::getDateFromEpoch($ts);
...
sub displayStatus {
	...
	if ($key = $root_key->get_subkey($key_path)){
		...
		foreach my $val (@vals) {
			...
			$d = $d." (".::getDateFromEpoch($d).")" if ($v ~~ @timestamp_fields);
...
sub displayFileFlags {
	...
	if ($key = $root_key->get_subkey($key_path)){
		...
		foreach my $val (@vals) {
			...
			my $str = $v.":\t".$d."(".::getDateFromEpoch($changetime[1]).")";
			::rptMsg($str);
		}
	}
	else {
...
sub displayIPM {
	...
	if ($key = $root_key->get_subkey($key_path)){
		...
		foreach my $val (@vals) {
			my $d = $val->get_data();
			my $v = $val->get_name();
			my $d = $d." (".::getDateFromEpoch($d).")"if ($v eq "ILU");
			my $str = $v.":\t".$d;
			::rptMsg($str);
		}
	}
	...
	if ($key = $root_key->get_subkey($key_path)){
		...
		foreach my $val (@vals) {
			...
			if ($v eq "Count"){
				...
				my $ts = $split_d[2];
				my $timestamp = ::getDateFromEpoch(hex($ts));
